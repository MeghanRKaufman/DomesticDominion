...
## Latest Change Log
- 2026-05-20: **Header roster fix** — "Party"/"Teammate" labels in the top-right header now derive from the actual `householdMembers` roster (returned by `/households/{id}/stats`) instead of the legacy `partnerId/partnerName` from the couples-app era. Also added a 15-second polling effect so newly joined members appear on the admin's dashboard without a manual refresh. New `data-testid`s: `header-party-badge`, `header-teammate-line`.
- 2026-05-14: **Epic Invite v2** — full Figma-grade redesign per `/app/design_guidelines.json`. Three distinct visual worlds: **EPIC** medieval scroll (Cinzel + IM Fell English serif, dark parchment, gold gradient code), **HYPE** neo-brutalist (Bricolage Grotesque, chartreuse #ccff00 + hard black borders, rotated stickers), **CHILL** pinboard (Fraunces + Caveat cursive handwritten sticky note). Google Fonts injected on mount.
- 2026-05-14: **Deep-link invites** — `?invite=CODE` URL param now auto-routes recipients to the join screen with the household preview banner (creator name, adventure theme, quest phrase, member count, household type, code chip) pre-loaded; recipients only enter their hero name. Query param is stripped after successful join. Verified in `/app/test_reports/iteration_9.json` (12/12 backend, 6/7 frontend critical flows green).
